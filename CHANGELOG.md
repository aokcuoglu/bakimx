...
Henüz yok — v0.9.2 tag'lendi, sonraki sürüm için birikmiş tag'siz geliştirme bulunmuyor.

## 0.9.x

| Sürüm | Başlık | Not |
|---|---|---|
| 0.9.2 | Katalog seed pipeline'ı onarıldı (TLS → bellek → task boyutu) + ölü kod temizliği | [v0.9.2](./docs/releases/v0.9.2.md) |
...
0.9.2 öne çıkanlar: her deploy'da koşan **araç kataloğu seed adımı, eklendiği 2026-07-24'ten beri hem prod hem dev'de sessizce başarısızdı** (non-blocking olduğu için yalnız bir uyarı satırı bırakıyordu). Üst üste üç kusur çıktı ve her biri bir öncekini çözünce göründü: RDS TLS doğrulaması (`self-signed certificate in certificate chain`, P1011) → fixture'ların tamamının belleğe alınması → 512 MB'lık one-off task'ın OOM ile öldürülmesi (exit 137). Katalog prod'a elle yüklendiği için kullanıcıya yansımıyordu; bozuk olan **kendi kendini iyileştirme**ydi. Ayrıca büyük bir temizlik: erişilemeyen bileşenler, **6 ulaşılamaz API rotası** (auth'lu ama ölü saldırı yüzeyi, dördü mutasyon yapıyordu), emekli PaddleOCR sidecar'ı ve hiç devreye alınmamış `@react-pdf/renderer` hattı (paket bağımlılıklardan düştü). Migration yok.
