...
export async function createAppointmentFromReminderAction(formData: FormData) {
  ...
  if (existing.createdAppointmentId) {
    return { redirect: `/app/appointments/${existing.createdAppointmentId}`, error: "Bu hatırlatma için zaten randevu oluşturulmuş" }
  }
  ...
  revalidatePath(`/app/reminders/${id}`)
  return { redirect: `/app/appointments/${appointment.id}` }
}
