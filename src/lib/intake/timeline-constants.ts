export const TIMELINE_EVENT_LABELS: Record<string, string> = {
  intake_created: "Kabul oluşturuldu",
  photos_uploaded: "Fotoğraflar yüklendi",
  damage_marks_added: "Hasar kayıtları eklendi",
  approval_requested: "Müşteri onayı istendi",
  approval_verified: "Müşteri onayı doğrulandı",
  work_order_created: "İş emri oluşturuldu",
  delivery_output_generated: "Teslim çıktısı oluşturuldu",
}

export const TIMELINE_EVENT_ICONS: Record<string, string> = {
  intake_created: "clipboard-list",
  photos_uploaded: "camera",
  damage_marks_added: "alert-triangle",
  approval_requested: "message-square",
  approval_verified: "check-circle",
  work_order_created: "wrench",
  delivery_output_generated: "package",
}