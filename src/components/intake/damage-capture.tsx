        ...
        <DialogContent>
          ...
          <Form {...form}>
            <form className="space-y-4" onSubmit={form.handleSubmit(save)}>
              ...
              <FormField control={form.control} name="damageType" render={({ field }) => <FormItem><FormLabel>Hasar türü</FormLabel><Select value={field.value} onValueChange={field.onChange}><FormControl><SelectTrigger className="w-full"><SelectValue>{(value: string | null) => value ? DAMAGE_TYPES[value as keyof typeof DAMAGE_TYPES]?.label : "Hasar türü seçin"}</SelectValue></SelectTrigger></FormControl><SelectContent>{Object.entries(DAMAGE_TYPES).map(([key, item]) => <SelectItem key={key} value={key}>{item.label}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>} />
              <FormField control={form.control} name="severity" render={({ field }) => <FormItem><FormLabel>Derece</FormLabel><Select value={field.value} onValueChange={field.onChange}><FormControl><SelectTrigger className="w-full"><SelectValue>{(value: string | null) => value ? DAMAGE_SEVERITY[value as keyof typeof DAMAGE_SEVERITY]?.label : "Derece seçin"}</SelectValue></SelectTrigger></FormControl><SelectContent>{Object.entries(DAMAGE_SEVERITY).map(([key, item]) => <SelectItem key={key} value={key}>{item.label}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>} />
